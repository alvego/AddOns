...
function Heal()
    ...
    if IsReadySpell("Длань возмездия") and UnitIsPlayer(target) and IsValidTarget(target) and (
      ((tContains(steathClass, GetClass(target))) and not InRange("Экзорцизм", target)) or UnitAffectingCombat(target) or HasBuff(reflectBuff, 1, target)
    ) and not HasDebuff("Длань возмездия", 1, target) and DoSpell("Правосудие света", target) then return end

    if IsReadySpell("Длань возмездия") and UnitIsPlayer(focus) and IsValidTarget(focus) and (
      (tContains(steathClass, GetClass(focus)) and not InRange("Экзорцизм", focus)) or UnitAffectingCombat(focus) or HasBuff(reflectBuff, 1, focus)
    ) and not HasDebuff("Длань возмездия", 1, focus) and DoSpell("Правосудие света", focus) then return end
