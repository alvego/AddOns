...
function Idle()
    ...
    if IsAttack() or InCombatLockdown() then
        ...
        if HasSpell("Удар воина Света") and  HasSpell("Печать повиновения") then
            Pve()
            return
        end
...
function TryBuffs()
    ...
    if HasSpell("Удар воина Света") then
        ...
        -- if HasBuff("Праведное неистовство") and orun("/cancelaura Праведное неистовство") then return end
...
function TryProtect()
    if InCombatLockdown() then
        ...
        if HasSpell("Удар воина Света") and HasSpell("Священная жертва") and (UnitHealth100() < 50) and DoSpell("Священная жертва") then return end
                if UnitHealth100() < 50 and orun("/cancelaura Священная жертва") then return end

        if GetTime() - tryShieldTime > 5 then
            if HasSpell("Удар воина Света") and (UnitHealth100() < 20) then
                if orun("/cast Божественный щит") then
