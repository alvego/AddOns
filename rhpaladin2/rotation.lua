...
-- dispels and heal-------------------------------------------------------------
  ...
  if CanMagicAttack(target) and IsAlt() and DoSpell("Правосудие справедливости", target) then return end
  ...
  if (InMelee(target) or DistanceTo(player, target) < 7) and (IsReadySpell("Божественная буря") or (GetSpellCooldownLeft("Божественная буря") < 0.5)) then
    DoSpell("Божественная буря")
    return
  end
  if CanMagicAttack(target) and DoSpell(((mana > 95) and "Правосудие света" or "Правосудие мудрости"), target) then return end
